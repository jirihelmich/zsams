...
class Daily_Tips_Table extends WP_List_Table
{
    ...
    public function prepare_items(){
		...
		if(isset($_REQUEST['display_date'])){
			$from_date = $_REQUEST['display_date'];
		}
		if(isset($_REQUEST['display_date'])){
			$to_date = $_REQUEST['display_date'];
		}
	    ...
	    <div class="metabox-holder">
			<div class="meta-box-sortables">
				...
				<div id="toc" class="postbox">
					...
					<p></p>
					<form id="edit_data" action="<?php echo $_SERVER['PHP_SELF']."?page=add-daily-tip"; ?>" method="post">
						<?php
						if (isset($_REQUEST['op'])&&isset($_REQUEST['edit_id'])) { 
				...
				<div class="handlediv" title="Click to toggle"><br /></div>
					...
					<form style="display:block; width:100%;">
						...
						<script type="text/javascript">
							...
							<input type="text" name="display_date" id="display_date" class="code" value="<?=$display_date;?>"/>
							<label>To</label>
							<input type="text" name="display_to_date" id="display_to_date" class="code" value="<?=$display_to_date;?>"/>
						</div>
						<input type="submit" class="button-primary" name="filter" value="Filter" />
					</form>
